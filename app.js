...
{                          // Raw velocity is in cells/iteration
  ...
  vel *= cellHeight;       // convert tos meters per hour
      ...
      for (var y = 0; y < sim_res_y; y++) {
        ...
        if (wallTextureValues[4 * y + 1] != 0) { // if this is fluid cell
          if (!reachedAir) {
            ...
            c.fillText('Sfc: ' + printAltitude(Math.round(map_range(y - 1, 0, sim_res_y, 0, guiControls.simHeight))), (T_to_Xpos(0,graphBottem)-20), (scrYpos + 17));
          ...
          //  c.fillText('6km Temp: ' + printTemp(temp_6k), (graphCanvas.width-220), 100);
            c.fillText('Sfc-3km AGL LR: ' + (Math.round(((temp-temp_3k)/3)*10)/10*1.1).toFixed(1) + '°C/km', (graphCanvas.width-220), 40);
            c.fillText('3-6km AGL LR: ' + (Math.round(((temp_3k-temp_6k)/3)*10)/10*1.1).toFixed(1) + '°C/km', (graphCanvas.width-220), 60); 
